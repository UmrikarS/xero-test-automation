...
## Skills demonstrated → job requirements

| Demonstrated here | Maps to |
|---|---|
| Page Object Model (`pages/`) | Playwright/Pytest course, general automation maturity |
| Equivalence partitioning & boundary value analysis (`test_invoice_amount_partitions`, both API and UI) | ISTQB test-design fundamentals |
| `@pytest.mark.parametrize` data-driven tests | "decorators" skill, data-driven testing |
| Test isolation via unique data + teardown fixtures (`fixtures/conftest.py`) | Robust, non-flaky test design for shared/live systems |
| UI-to-data reconciliation test (`test_aged_receivables_total_matches...`) | Foodstuffs — data validation across systems |
| API contract/data-quality tests (`TestContactDataQuality`, `test_created_invoice_total_matches_line_items`) | Foodstuffs — API testing, SQL/data validation mindset |
| OWASP-based SQL injection suite, vulnerable-vs-safe comparison | PrivSec Consulting / Layer 9 Security — security testing mindset |
| GitHub Actions CI (`.github/workflows/tests.yml`) | CI/CD exposure |
| OAuth2 Custom Connection handling (`utils/xero_auth.py`) | Real-world API integration, not just a happy-path demo |
| Postman collection (`postman/`) | Manual/exploratory testing tooling |
| AI-assisted test case generation + data verification, with visible human review (`ai_assisted_testing/`) | "Learn how AI can assist in test case generation and data verification" — direct match for QA/testing internship roles |
